...
import {
  Button,
  Responsive as ButtonResponsive,
...
export default () => {
  ...
  useEffect(() => {
    if (visibleNav) {
      gsap.to("#nav-movil", { duration: 0.25, left: 0 });
    } else {
      gsap.to("#nav-movil", { duration: 0.25, left: "100vw" });
    }
  }, [visibleNav]);
  ...
  return (
    <MainCont>
      ...
      <NavContainer id='nav-movil'>
          ...
          <Container>
            ...
            <NavItem>
              <Link to={`/contact?builderId=${builderId}`}>
                <NavLink noMargin>Contacto</NavLink>
              </Link>
            </NavItem>
          </Container>
        </NavBar>
        ...
        <Container style={{ width: "100%" }}>
          <SocialCont>
            <SocialItem>Siguenos</SocialItem>
            <SocialItem>
